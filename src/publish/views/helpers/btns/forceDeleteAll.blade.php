...
@if (auth()->user()->rule != 'admin')
 ...
 <label for="submitForceDeleteAll" 

	@foreach($attr as $key => $value)
	{{ $key }}="{{ $value }}" &nbsp

	@endforeach
 ><i class="fa fa-times fa-fw"></i> {{ trans('lang.force_delete_selected') }}</label>

	

@else
	   <label for="submitForceDeleteAll" class="btn btn-danger"><i class="fa fa-times fa-fw"></i> {{ trans('lang.force_delete_selected') }}</label>
    

@endif

{!! Form::open(['method'=>'delete','id'=>'formForceDeleteAll','class'=>'hidden']) !!}
    {!! Form::submit('',['class'=>'hidden','id'=>'submitForceDeleteAll']) !!}
    {!! Form::close() !!}
