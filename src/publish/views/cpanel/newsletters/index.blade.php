...
@section('menu') {!! getBreadcrumbs('newsletter')->index !!}  @endsection
@section('content')
  ...
  {!! Btn::deleteAll() !!}  {!! Btn::trashed() !!} 
    ...
    <div class="portlet-body">
        <table class="table table-striped table-bordered table-hover datatable" width="100%">
            ...
            <tr>
                <td class="text-center">{!! bsForm::deleteSelect($newsletter->id) !!}</td>
                 <td>{{ $newsletter->email }}</td>
                 <td>{{ date('Y/m/d ( h:i:s A)',strtotime($newsletter->created_at)) }}</td>
                 <td>
                     {!! Btn::delete($newsletter->id,$newsletter->email) !!}
                 </td>
            </tr>
            @endforeach
