...
try {
	...
	$trashedUrl = isset($urls['trashed']) ? $urls['trashed'] : url(($cp ? cp : '').str_plural($name).'/get/trashed');
	...
	Breadcrumbs::register(str_plural($name).'.trashed', function($breadcrumbs) use($name,$trashedUrl){
	    $breadcrumbs->parent(str_plural($name));
	    $breadcrumbs->push(trans('lang.trashed'), $trashedUrl);
	});
	...
	Breadcrumbs::register(str_plural($name).'.show_trashed', function($breadcrumbs,$id) use($name,$model,$colum ){
		$showUrl = isset($urls['show_trashed']) ? 
		$urls['show_trashed'] : action(getController().'@showTrashed',$id);
	    $breadcrumbs->parent(str_plural($name).'.trashed');
	    $query = $model::onlyTrashed()->where('id',$id)->first();
	    if (is_callable($colum)) 
	    {
	    	$dbname = @call_user_func_array($colum, [$query]);
	    }else{
...
function getBreadcrumbs($name,$id = null)
{
	...
	return (object)[
		...
		'trashed' => Breadcrumbs::renderIfExists(str_plural($name).'.trashed'),
		'show_trashed' => Breadcrumbs::renderIfExists(str_plural($name).'.show_trashed',$id),
	];
}